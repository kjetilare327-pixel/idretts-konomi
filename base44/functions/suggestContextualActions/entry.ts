...
Deno.serve(async (req) => {
  try {
    ...
    const { context, team_id } = body;

    if (!context || !team_id) return Response.json({ error: 'context and team_id required' }, { status: 400 });

    // Require admin, kasserer, or styreleder
    if (user.role !== 'admin') {
    ...
    if (context === 'composing_message') {
      ...
      if (overdueClaims.length > 0) {
        ...
        if (paymentTemplate) {
          suggestions.push({ type: 'template', title: 'Bruk betalingspåminnelse-mal', description: `Du har ${overdueClaims.length} forfalte betalinger.`, action: 'use_template', data: paymentTemplate });
        }
      }
      ...
      if (upcomingEvents.length > 0) {
        ...
        if (eventTemplate) {
          suggestions.push({ type: 'template', title: 'Varsle om kommende arrangement', description: `Du har ${upcomingEvents.length} kommende arrangementer.`, action: 'use_template', data: eventTemplate });
        }
      }
    }
    ...
    if (context === 'viewing_dashboard') {
      ...
      if (totalBudget > 0 && totalMonthlyExpenses > totalBudget * 0.8) {
        suggestions.push({ type: 'warning', title: 'Utgifter nærmer seg budsjett', description: `Du har brukt ${((totalMonthlyExpenses / totalBudget) * 100).toFixed(0)}% av budsjettet denne måneden.`, action: 'view_budget', data: { url: '/Budget' } });
      }

      if (claims.length > 0) {
        suggestions.push({ type: 'action', title: 'Send betalingspåminnelser', description: `${claims.length} betalinger er forfalt. Send påminnelser nå?`, action: 'send_reminders', data: { claim_count: claims.length } });
      }
    }
    ...
    if (context === 'viewing_player_profile') {
      ...
      if (player_id) {
        const [player, playerClaims] = await Promise.all([
          base44.asServiceRole.entities.Player.get(player_id),
          base44.asServiceRole.entities.Claim.filter({ player_id })
        ]);

        if (player.balance > 0) {
          suggestions.push({ type: 'action', title: 'Opprett betalingskrav', description: `Spilleren skylder ${player.balance} kr.`, action: 'create_claim', data: { player_id, amount: player.balance } });
        }

        const unpaidClaims = playerClaims.filter(c => c.status === 'pending' || c.status === 'overdue');
        if (unpaidClaims.length > 0) {
          suggestions.push({ type: 'action', title: 'Send betalingspåminnelse', description: `Spilleren har ${unpaidClaims.length} ubetalte krav.`, action: 'send_reminder', data: { player_id, claims: unpaidClaims } });
        }
      }
    }
