...
Foreslå et optimalt budsjett. Svar på norsk.`;
    ...
    const aiResponse = await base44.asServiceRole.integrations.Core.InvokeLLM({
      ...
      response_json_schema: {
        type: 'object',
        properties: {
          recommended_budgets: { type: 'array', items: { type: 'object', properties: { category: { type: 'string' }, type: { type: 'string', enum: ['income', 'expense'] }, monthly_amount: { type: 'number' }, yearly_amount: { type: 'number' }, reasoning: { type: 'string' } } } },
          optimization_opportunities: { type: 'array', items: { type: 'object', properties: { area: { type: 'string' }, current_spend: { type: 'number' }, potential_savings: { type: 'number' }, action: { type: 'string' }, priority: { type: 'string', enum: ['high', 'medium', 'low'] } } } },
          liquidity_forecast: { type: 'object', properties: { next_3_months: { type: 'number' }, next_6_months: { type: 'number' }, next_12_months: { type: 'number' }, risk_level: { type: 'string', enum: ['low', 'medium', 'high'] }, description: { type: 'string' } } },
          recommendations: { type: 'array', items: { type: 'object', properties: { title: { type: 'string' }, description: { type: 'string' }, impact: { type: 'string', enum: ['high', 'medium', 'low'] }, effort: { type: 'string', enum: ['high', 'medium', 'low'] } } } },
          summary: { type: 'string' }
        }
      }
    });
    ...
    return Response.json({
      success: true, budget_plan: aiResponse,
      historical_data: { last_year_income: lastYearIncome, last_year_expense: lastYearExpense, income_growth: incomeGrowth, expense_growth: expenseGrowth }
    });
