...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Step A: Check TeamMember via SERVICE ROLE
    let memberRecord = null;
    let memberFound = false;
    try {
      const byTeam = await base44.asServiceRole.entities.TeamMember.filter({ team_id });
      memberRecord = byTeam.find(m => m.user_email === userEmail && m.status === 'active') || null;
      memberFound = !!memberRecord;
      console.log(`[getJoinStatus] ${requestId} stepA: byTeam.count=${byTeam.length} memberFound=${memberFound} recordId=${memberRecord?.id || 'none'}`);
    } catch (e) {
